# Documentation

Welcome to the Functional State Machine docs. Each guide introduces a feature, why it is useful, and simple to advanced examples.

## Getting Started

- [Packages and when to reference them](Packages.md)
- [AI Usage Guide](../AI-USAGE-GUIDE.md) - Quick reference for AI coding assistants
- [Commands instead of side effects](Commands-Instead-of-Side-Effects.md)

## Core Features

- [Fluent configuration](Fluent-Configuration.md)
- [State data and ModifyData](State-Data-and-ModifyData.md)
- [Guards and conditional flows](Guards-and-Conditional-Flows.md)
- [Conditional steps (If/ElseIf/Else)](Conditional-Steps.md)
- [Entry and exit commands](Entry-and-Exit-Commands.md)
- [Execute steps and multiple commands](Execute-Steps-and-Multiple-Commands.md)
- [Internal transitions](Internal-Transitions.md)
- [Immediate transitions](Immediate-Transitions.md)
- [Hierarchical states](Hierarchical-States.md)
- [Ignore and unhandled triggers](Ignore-and-Unhandled-Triggers.md)
- [No-data builder](No-Data-Builder.md)

## Validation and Analysis

- [Static analysis and configuration validation](Static-Analysis-for-State-Machine-Configuration.md)

## Tooling and Integrations

- [Mermaid diagram generation](Mermaid-Diagram-Generation.md)
- [Command runners](Command-Runners.md)
