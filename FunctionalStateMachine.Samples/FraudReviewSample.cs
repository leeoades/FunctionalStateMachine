...
public static class FraudReviewSample
{
    public static StateMachine<ReviewState, ReviewTrigger, ReviewData, ReviewCommand> Build()
    {
        ...
        pending.On(ReviewTrigger.Submit)
            .Guard((state, trigger) => state.Data.RiskScore > 70)
        ...
        pending.On(ReviewTrigger.Submit)
            ...
            .Execute((State<ReviewState, ReviewData> state) => new AuditReviewCommand($"Case:{state.Data.CaseId}"))
            .Execute((state, trigger) => new NotifyCommand($"Approved {state.Data.CaseId}"));
        ...
        builder.For(ReviewState.Manual)
            .On(ReviewTrigger.Approve)
                ...
                .Execute(() => new ReviewCommand[] { new NotifyCommand("Manual approved"), new AuditReviewCommand("Manual path") });
